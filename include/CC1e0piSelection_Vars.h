...
namespace ana {
    ...
    const double THRESHOLD_E = 0.2; // 0.2 0.075; // experimental -- 75 MeV
    ...
    const Var kHaveMuonCandidate([](const caf::SRSliceProxy *slc) -> bool {
       ...
       // TVector3 recoVertex(slc->vertex.x, slc->vertex.y, slc->vertex.z);
        ...
        for (const auto& pfp : slc->reco.pfp) {
            ...
            //recoStart.SetXYZ(pfp.trk.start.x, pfp.trk.start.y, pfp.trk.start.z);
            ...
            double chi2p = pfp.trk.chi2pid[bestPlane].chi2_proton;
            double chi2m = pfp.trk.chi2pid[bestPlane].chi2_muon;

            if (chi2m < 30) continue; // muon-like
            if (chi2p > 60) continue; // not proton-like

            if (pfp.trk.end.x * slc->vertex.x <= 0.) continue;
            if (!pfp.parent_is_primary) continue;
            if (pfp.trk.len > highestLength) {
                highestLength = pfp.trk.len;
            }
        }

        return (highestLength > 0);
    }); 
    ...
    const MultiVar kNSelectedProtonsIdx([](const caf::SRSliceProxy* slc) -> std::vector<double> {
        ...
        // Trova lo shower principale (elettrone)
        const int largestShwIdx = kLargestRecoShowerIdx(slc);
        if (largestShwIdx == -1) return selectedProtonIdx;

        for (unsigned int i = 0; i < slc->reco.npfp; i++) {

            // Salta lo shower principale
            if (i == (unsigned int) largestShwIdx) continue;

            auto const& pfp = slc->reco.pfp[i]; // necessario per calcolare il best plane
            int bestPlane = pfp.trk.bestplane;

            if (bestPlane < 0) continue;

            double ts = slc->reco.pfp[i].trackScore;

            // Calcola chi² proton sul best plane
            double chi2p = slc->reco.pfp[i].trk.chi2pid[bestPlane].chi2_proton;

            // TS > 0.5 → track-like
            if (ts > 0.5) {
                if (chi2p < 100) { // proton-like (HIP)
                    if (kIsPFPProtonLike(slc, i)) selectedProtonIdx.push_back(i);
                } else { // pion-like
                    if (kIsPFPPionLike(slc, i)) NOtherParticles += 1;
                }
            } 
            // TS < 0.5 → contenuti / shower-like
            else {
                if (ts > 0.45) { // borderline proton-like
                    if (kIsPFPProtonLike(slc, i)) selectedProtonIdx.push_back(i);
                } else { // shower-like
    ...
    const Var kNSelectedProtonsIdx_NOtherParticles([](const caf::SRSliceProxy* slc) -> int { 
        ...
        // Trova lo shower principale (elettrone)
        const int largestShwIdx = kLargestRecoShowerIdx(slc);
        if (largestShwIdx == -1) return NOtherParticles;
        ...
        for (unsigned int i = 0; i < slc->reco.npfp; i++) {
            ...
            auto const& pfp = slc->reco.pfp[i]; // necessario per calcolare il best plane
            int bestPlane = pfp.trk.bestplane; // (shw.plane[1].nHits > shw.plane[2].nHits) ? 1 : 2;
            ...
            if (ts > 0.5) {
                if (chi2p < 100) { // proton-like (HIP)
                    if (kIsPFPProtonLike(slc, i)) continue;
                } else { // pion-like
                    if (kIsPFPPionLike(slc, i)) NOtherParticles += 1;
                }
            } 
